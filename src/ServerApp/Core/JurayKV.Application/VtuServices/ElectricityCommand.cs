        ...
        public async Task<ElectricityResponse> Handle(ElectricityCommand request, CancellationToken cancellationToken)
        {
            ...
            if (result.code == "success")
            {
                try
                {
                    ...
                    //create transaction
                    CreateTransactionCommand createtransaction = new CreateTransactionCommand(userwallet.Id, userwallet.UserId, "ELECTRICITY", Convert.ToDecimal(request.Amount), TransactionTypeEnum.Debit, EntityStatus.Successfull, result.data.order_id, "ELECTRICITY PURCHASE <br>" + result.data.token, result.data.order_id);
